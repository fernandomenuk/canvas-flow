...
function setupHooksEnv(homeDir, stateDir) {
  ...
  return { ...env, HOME: homeDir, CANVAS_FLOW_STATE_DIR: stateDir };
}

test("CLI version tracks package.json so version bumps reach the published binary", async () => {
  const packageJson = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
  assert.equal(VERSION, packageJson.version);
});

test("home output teaches agents when and how to use Canvas Flow", () => {
  const output = createHomeOutput({ bin: `${os.homedir()}/.local/bin/canvas-flow`, sessions: [] });

  assert.equal(output.bin, "~/.local/bin/canvas-flow");
  assert.match(output.description, /Canvas Flow/);
  assert.match(output.description, /complex response/);
  assert.match(output.description, /consider using Canvas Flow/);
  ...
  assert.ok(output.help.some((item) => item.includes("canvas-flow <html-file>")));
  assert.ok(output.help.some((item) => item.includes("`.canvasflow/`")));
  assert.ok(output.help.some((item) => item.includes("canvas-flow playbook <playbook_id>")));
  ...
  assert.ok(output.help.some((item) => item.includes("canvas-flow design")));
  ...
  assert.ok(!output.help.some((item) => item.includes('<meta name="canvasflow-design" content="off">')));
...
test("home output warns agents that poll is a long poll they must not kill", () => {
  const output = createHomeOutput({ bin: "canvas-flow", sessions: [] });
  const pollHelp = output.help.find((item) => item.includes("canvas-flow poll <html-file>"));
...
test("top-level help renders static home output without dynamic sessions", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/canvas-flow-help-test-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url)), "--help"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
        env: { ...process.env, CANVAS_FLOW_STATE_DIR: stateDir },
      },
    );

    assert.equal(result.status, 0, result.stderr || result.stdout);
    assert.match(result.stdout, /playbooks\[7\]/);
    assert.match(result.stdout, /canvas-flow playbook <playbook_id>/);
    ...
    assert.match(result.stdout, /canvas-flow design/);
    ...
    assert.doesNotMatch(result.stdout, /canvasflow-design/);
...
test("playbook detail output returns focused CanvasFlow-native guidance", () => {
  ...
  assert.ok(output.playbook.design_rules.some((item) => item.includes("data-canvasflow-action")));
  assert.ok(output.playbook.design_rules.some((item) => item.includes("data-canvasflow-question")));
  assert.ok(output.playbook.design_rules.some((item) => item.includes("queueKey")));
  assert.ok(output.playbook.canvasflow_notes.some((item) => item.includes("window.canvasflow.queuePrompt")));
  assert.ok(output.playbook.canvasflow_notes.some((item) => item.includes("onsubmit")));
  ...
  assert.ok(output.playbook.canvasflow_notes.some((item) => item.includes("CanvasFlow")));
});
...
test("unknown playbook ids produce an actionable validation error", () => {
  assert.throws(
    ...
    (error) => {
      ...
      assert.ok(error.suggestions.some((item) => item.includes("canvas-flow playbook")));
      return true;
    },
  );
});
...
test("home directory collapse tolerates Windows mixed separators", () => {
  assert.equal(
    collapseHomeDirectory("C:\\Users\\runneradmin/.local/bin/canvas-flow", "C:\\Users\\runneradmin"),
    "~/.local/bin/canvas-flow",
  );
  assert.equal(
    collapseHomeDirectory("C:\\Users\\runneradmin\\.local\\bin\\canvas-flow", "C:\\Users\\runneradmin"),
    "~/.local/bin/canvas-flow",
...
test("open output keeps the user URL in session data and next_step focused on polling", () => {
  ...
  assert.match(output.next_step, /canvas-flow poll \/tmp\/artifact\.html/);
...
test("a user-ended open refuses with a status agents can branch on, not a URL to open", () => {
  ...
  assert.match(output.next_step, /user explicitly ended this Canvas Flow session from the browser/);
  ...
  assert.match(output.next_step, /canvas-flow \/tmp\/artifact\.html --reopen/);
});
...
test("export output reports the written file and reassures it needs no server", () => {
  ...
  assert.match(output.next_step, /no CanvasFlow server/);
...
test("export command writes a portable HTML file next to the artifact", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/canvas-flow-export-test-`);
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url)), "export", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, CANVAS_FLOW_STATE_DIR: dir, CANVAS_FLOW_TELEMETRY: "0" },
        encoding: "utf8",
      },
    );
...
test("export command treats --out value as an option operand, not the source file", async () => {
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url)), "export", "--out", output, artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, CANVAS_FLOW_STATE_DIR: dir, CANVAS_FLOW_TELEMETRY: "0" },
        encoding: "utf8",
...
test("share output reports the public url and the secret update key", () => {
  ...
  assert.match(output.next_step, /ht-ml\.app \(https:\/\/ht-ml\.app\), a third-party host not part of CanvasFlow/);
});
...
test("share command publishes the artifact to ht-ml.app and returns the public url", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/canvas-flow-share-test-`);
  const artifact = `${dir}/report.html`;
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url)), "share", "--password", "pw", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: {
          ...process.env,
          CANVAS_FLOW_STATE_DIR: dir,
          CANVAS_FLOW_TELEMETRY: "0",
          CANVAS_FLOW_HTML_APP_API_URL: `http://127.0.0.1:${htmlApp.port}`,
        },
      },
    );
    let stdout = "";
...
test("share command treats a whitespace-only password as public", async () => {
  ...
  try {
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url)), "share", "--password", "   ", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: {
          ...process.env,
...
test("share help distinguishes public default from password-protected shares", () => {
  ...
  const home = createHomeOutput({ bin: "canvas-flow", sessions: [] });
  const homeShareHelp = home.help.find((item) => item.includes("canvas-flow share <html-file>"));
...
test("a poll reporting the session ended by the user tells the agent to stop and not reopen", () => {
  ...
  assert.match(output.next_step, /user ended this Canvas Flow session/);
  assert.match(output.next_step, /Stop polling/);
  assert.match(output.next_step, /do not run `canvas-flow \/tmp\/report\.html` to reopen it/);
  assert.match(output.next_step, /deliver any remaining updates directly in this conversation/i);
  assert.match(output.next_step, /canvas-flow \/tmp\/report\.html --reopen/);
});
...
test("a poll reporting an agent-ended session allows a plain reopen if still needed", () => {
  ...
  assert.match(output.next_step, /Stop polling/);
  assert.match(output.next_step, /canvas-flow \/tmp\/report\.html`\s+to open a fresh session/);
...
test("the final feedback batch before an agent end preserves ended_by and allows plain reopen", () => {
  ...
  assert.equal(output.session.session_ended, true);
  assert.equal(output.session.ended_by, "agent");
  assert.match(output.next_step, /last feedback before the Canvas Flow session ended/);
  assert.match(output.next_step, /canvas-flow \/tmp\/report\.html`\s+to open a fresh session/);
  assert.doesNotMatch(output.next_step, /--reopen/);
  assert.doesNotMatch(output.next_step, /user ended this Canvas Flow session/);
});
...
test("poll wait messages tell watching agents the silence is normal", () => {
  ...
  assert.match(banner, /\[canvas-flow\]/);
  ...
  assert.match(interrupted, /\[canvas-flow\]/);
  assert.match(interrupted, /Poll interrupted/);
  assert.match(interrupted, /user may still be reviewing/);
  assert.match(interrupted, /canvas-flow poll \/tmp\/report\.html/);
...
test("spawned poll announces the wait on stderr and leaves re-run guidance when killed", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/canvas-flow-poll-wait-test-`);
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url)), "poll", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, CANVAS_FLOW_STATE_DIR: stateDir, CANVAS_FLOW_PORT: String(server.port) },
      },
    );
...
test("waiting next step reassures agents that re-running poll loses nothing", () => {
  ...
  assert.match(output.next_step, /canvas-flow poll \/tmp\/report\.html/);
  assert.match(output.next_step, /without --timeout-ms/);
  assert.match(output.next_step, /queued feedback is never lost/);
});
...
test("setup hooks resolves HOME before platform-specific user profile variables", () => {
  assert.equal(
    resolveHookHomeDir({ HOME: "/tmp/canvasflow-home", USERPROFILE: "C:\\Users\\runneradmin" }, "/fallback"),
    "/tmp/canvasflow-home",
  );
});
...
test("setup hooks creates a Copilot CLI hook that injects additional context", () => {
  ...
  assert.match(updated.hooks.sessionStart[1].bash, /canvas-flow/);
...
test("Copilot CLI ambient context script wraps canvasflow output as hook JSON", async () => {
  const tempDir = await mkdtemp(`${os.tmpdir()}/canvas-flow-copilot-hook-`);
  try {
    const fakeCli = path.join(tempDir, "fake-canvasflow.js");
    ...
    assert.match(output.additionalContext, /## AXI ambient context: canvas-flow/);
...
test("setup hooks installs agent session hooks explicitly", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/canvas-flow-setup-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/canvas-flow-setup-home-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url)), "setup", "hooks"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
    ...
    assert.ok(existsSync(`${homeDir}/.copilot/hooks/canvas-flow.json`));

    const copilotHook = JSON.parse(await readFile(`${homeDir}/.copilot/hooks/canvas-flow.json`, "utf8"));
...
test("setup hooks exits with an error when hook installation fails", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/canvas-flow-setup-fail-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/canvas-flow-setup-fail-home-`);
  try {
...
test("server entry resolves to a node-executable script that actually invokes run()", () => {
  // Running from source, the entry must be `bin/canvas-flow.js` (the only file in the
  ...
  // From source: bin/canvas-flow.js is present and preferred.
  assert.equal(entry, fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url)));
});
...
test("local built CLI opens force a server restart while source and installed runs do not", () => {
  ...
  assert.equal(shouldForceRestartForLocalBuild(`${root}/bin/canvas-flow.js`, true), false);
  assert.equal(shouldForceRestartForLocalBuild("/usr/local/lib/node_modules/canvas-flow/dist/cli.mjs", false), false);
});
...
test("shouldRestartServer restarts same-version CanvasFlow servers when forced", () => {
  assert.equal(shouldRestartServer("0.1.4", { ok: true, app: "canvas-flow", version: "0.1.4" }, true), true);
...
test("shouldKillProcessOnPort kills pre-handshake CanvasFlow servers after shutdown fails", () => {
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true }), true);
});

test("shouldKillProcessOnPort only kills CanvasFlow servers with a mismatched version", () => {
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true, app: "canvas-flow", version: "0.1.3" }), true);
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true, app: "canvas-flow", version: "0.1.4" }), false);
});

test("shutdownServerOnPort kills pre-handshake CanvasFlow servers when shutdown does not free the port", async () => {
  let shutdowns = 0;
  let kills = 0;
...
test("shutdownServerOnPort ignores unidentified health responders", async () => {
  ...
  const output = await shutdownServerOnPort(4387, {
    ...
    processMatchesCanvasFlow: () => false,
  });

  assert.equal(shutdowns, 0);
  assert.equal(kills, 0);
  assert.deepEqual(output, { server: { status: "not-canvasflow", port: 4387 } });
});
...
test("open can resume a session without opening another browser window", () => {
  ...
  assert.equal(shouldOpenBrowser(["artifact.html"], { CANVAS_FLOW_NO_OPEN: "1" }), false);
  ...
  assert.match(getCommandHelp("design"), /canvas-flow design/);
...
test("polling a file without an active session tells the agent to open it first", () => {
  assert.throws(
    ...
    (error) => {
      ...
      assert.match(error.message, /No active Canvas Flow session/);
      assert.ok(error.suggestions.some((item) => item.includes("canvas-flow /tmp/report.html")));
      return true;
    },
  );
});

test("network fetch failures become structured CanvasFlow server errors", async () => {
  await assert.rejects(
    () => fetchJson("http://127.0.0.1:1/api/poll"),
    (error) => {
      assert.ok(error instanceof AxiError);
      assert.equal(error.code, "SERVER_ERROR");
      assert.match(error.message, /Canvas Flow server connection failed/);
      assert.ok(error.suggestions.some((item) => item.includes("canvas-flow server --verbose")));
      return true;
    },
  );
});
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) => {
        assert.ok(error instanceof AxiError);
        assert.equal(error.code, "SERVER_ERROR");
        assert.match(error.message, /Canvas Flow poll response was interrupted/);
        return true;
      },
    );
...
test("stop command shuts down the running server on the configured port", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/canvas-flow-stop-test-`);
