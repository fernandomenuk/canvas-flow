...
export const CANVAS_FLOW_INTERNAL_QUEUE_KEY = "_canvasflowQueueKey";
...
export function deriveCanvasFlowQueueKey(element, options = {}) {
  ...
  function scopeKey(el) {
    ...
    const explicit = stringValue(
      attributeValue(scope, "data-canvasflow-question") || attributeValue(scope, "id") || attributeValue(scope, "name"),
    ).trim();
  ...
  const question = closestElementMatching(element, "[data-canvasflow-question]");
  const questionKey = stringValue(attributeValue(question, "data-canvasflow-question")).trim();
...
export function createArtifactSdk(
  ...
  function textSelectionContext(selection) {
    ...
    if (isCanvasFlowUi(ancestor) || isCanvasFlowAction(ancestor) || isInteractiveControl(ancestor)) return null;
  ...
  function isCanvasFlowUi(el) {
    return !!(el && el.closest && el.closest("[data-canvasflow-ui]"));
  }

  function isCanvasFlowAction(el) {
    return !!(el && el.closest && el.closest("[data-canvasflow-action]"));
  }
  ...
  function clearTextHighlight() {
    if (!shadow) return;
    for (const el of [...shadow.querySelectorAll(".canvasflow-text-highlight")]) el.remove();
  }
  ...
  function highlightTextRange(range) {
    ...
    for (const rect of [...range.getClientRects()]) {
      ...
      mark.className = "canvasflow-text-highlight";
  ...
  function queuePrompt(prompt, options = {}) {
    ...
    /** @type {{ uid: string, prompt: string, selector: string, tag: string, text: string, target?: unknown, _canvasflowQueueKey?: string }} */
    const item = {
      ...context(originElement),
      prompt: String(prompt || ""),
    };
    const queueKey = typeof deriveQueueKey === "function" ? deriveQueueKey(originElement, options) : "";
    if (queueKey) item._canvasflowQueueKey = String(queueKey);
    ...
    parent.postMessage({ type: "canvasflow:queuePrompt", prompt: item }, "*");
  }

  function sendQueuedPrompts() {
    parent.postMessage({ type: "canvasflow:sendQueuedPrompts" }, "*");
  }

  function endSession() {
    parent.postMessage({ type: "canvasflow:endSession" }, "*");
  }
  ...
  function snapshot() {
    ...
    function walk(el, depth) {
      if (!(el instanceof Element) || depth > 6 || isCanvasFlowUi(el)) return;
  ...
  function isVisibleForLayoutAudit(el, rect = el.getBoundingClientRect()) {
    if (!el || isCanvasFlowUi(el) || rect.width <= 0 || rect.height <= 0) return false;
  ...
  function collectLayoutAuditElements() {
    ...
    function walk(el) {
      if (!(el instanceof Element) || isCanvasFlowUi(el)) return;
    ...
    for (const el of candidates) {
      ...
      for (const rect of fragments) {
        ...
        for (const point of points) {
          ...
          if (!(top instanceof Element) || top === el || el.contains(top) || top.contains(el) || isCanvasFlowUi(top))
            continue;
  ...
  async function runLayoutAudit(runId) {
    ...
    parent.postMessage({ type: "canvasflow:layoutWarnings", layout_warnings }, "*");
  }
  ...
  function ensureShadow() {
    ...
    host.className = "canvasflow-annotation-root";
    host.setAttribute("data-canvasflow-ui", "annotation-root");
  ...
  function closeCard() {
    if (shadow) {
      for (const el of [...shadow.querySelectorAll(".canvasflow-annotation-card")]) el.remove();
    }
  ...
  function showAnnotationCard(target, options = {}) {
    ...
    card.className = "canvasflow-annotation-card";
    ...
    card.innerHTML =
      '<div class="canvasflow-heading">' +
      heading +
      '</div><textarea placeholder="' +
      placeholder +
      '"></textarea><div class="canvasflow-hint">Enter to queue &middot; ' +
      (/Mac|iP(hone|ad|od)/.test(navigator.platform) ? "⌘" : "Ctrl") +
      '+Enter to send now</div><div class="canvasflow-row"><button class="canvasflow-cancel" type="button">Cancel</button><button class="canvasflow-send" type="button">Queue</button></div>';
    ...
    const cancelButton = /** @type {HTMLButtonElement | null} */ (card.querySelector(".canvasflow-cancel"));
    const sendButton = /** @type {HTMLButtonElement | null} */ (card.querySelector(".canvasflow-send"));
  ...
  /** @type {Window & { canvasflow?: unknown }} */ (window).canvasflow = {
    queuePrompt,
    sendQueuedPrompts,
    endSession,
    getQueuedPrompts: () => [],
    setStatus: (message) => parent.postMessage({ type: "canvasflow:status", message: String(message) }, "*"),
    snapshot,
  };

  window.addEventListener("message", (event) => {
    const msg = event.data || {};
    if (msg.type === "canvasflow:setAnnotationMode") setAnnotationMode(msg.enabled);
    if (msg.type === "canvasflow:requestSnapshot") {
      parent.postMessage({ type: "canvasflow:snapshot", snapshot: snapshot() }, "*");
    }
    if (msg.type === "canvasflow:restoreScroll") {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      event.preventDefault();
      parent.postMessage({ type: "canvasflow:toggleAnnotationMode" }, "*");
    },
    true,
  );
  ...
  window.addEventListener(
    ...
    () => {
      ...
      scrollFrame = window.requestAnimationFrame(() => {
        scrollFrame = 0;
        parent.postMessage({ type: "canvasflow:scroll", x: window.scrollX, y: window.scrollY }, "*");
      });
    },
    { passive: true },
  );
  ...
  document.addEventListener(
    ...
    (event) => {
      if (
        !annotationMode ||
        isCanvasFlowUi(event.target) ||
        isCanvasFlowAction(event.target) ||
