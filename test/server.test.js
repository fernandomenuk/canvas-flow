...
test("server serves chrome browser behavior from a dedicated source file", async () => {
  ...
  assert.match(html, /<script id="canvasflow-session" type="application\/json">/);
...
test("artifact assets resolve within the artifact directory", () => {
  const root = path.resolve("/tmp/canvasflow-artifact");
...
test("artifact SDK uses a custom annotation card instead of browser prompts", () => {
  ...
  assert.match(js, /canvasflow-annotation-card/);
...
test("artifact SDK ignores CanvasFlow-owned annotation UI", () => {
  const js = createSdkJs("abc");

  assert.match(js, /function isCanvasFlowUi/);
  assert.match(js, /closest\(["']\[data-canvasflow-ui\]["']\)/);
  assert.match(js, /data-canvasflow-ui/);
});

test("artifact SDK isolates CanvasFlow annotation UI in Shadow DOM", () => {
  const js = createSdkJs("abc");

  assert.match(js, /attachShadow\(\{\s*mode:\s*["']open["'],?\s*\}\)/);
  assert.match(js, /:host\{all:initial/);
  assert.match(js, /canvasflow-annotation-root/);
});
...
test("annotation mode forces the artifact cursor to default", () => {
  ...
  assert.match(js, /canvasflow-cursor-style/);
...
test("artifact SDK registers a capture-phase document keydown listener for the mode toggle hotkey", () => {
  ...
  assert.match(js, /parent\.postMessage\(\{ type: "canvasflow:toggleAnnotationMode" \}, "\*"\);/);
  ...
  assert.match(
    js,
    /document\.addEventListener\(\s*"keydown",\s*\(event\) => \{\s*if \(!isModeToggleHotkeyEvent\(event\)\) return;\s*event\.preventDefault\(\);\s*parent\.postMessage\(\{ type: "canvasflow:toggleAnnotationMode" \}, "\*"\);\s*\},\s*true,?\s*\);/,
  );
});
...
test("chrome client toggles annotation mode via Cmd/Ctrl+I and on request from the artifact SDK", async () => {
  ...
  assert.match(js, /if \(msg\.type === "canvasflow:toggleAnnotationMode"\) toggleAnnotationMode\(\);/);
  assert.match(
    js,
...
test("artifact SDK lets marked feedback controls handle their own clicks", () => {
  ...
  assert.match(js, /function isCanvasFlowAction/);
  assert.match(js, /closest\(["']\[data-canvasflow-action\]["']\)/);
  assert.match(js, /isCanvasFlowAction\(event\.target\)/);
  assert.match(js, /\[data-canvasflow-action\],[^{}]*\[data-canvasflow-action\] \*\{cursor:pointer!important\}/);
});
...
test("annotation card shadow styles use CanvasFlow design-system variables", () => {
  const js = createSdkJs("abc");

  assert.match(js, /--bg:#0b0e14/);
  assert.match(js, /--accent:#6d5cff/);
...
test("annotate switch shows an accent track and ink knob when enabled", async () => {
  const js = await chromeClientSource();
...
test("chrome declares the CanvasFlow design-system tokens", async () => {
  const css = await chromeCssSource();

  assert.match(css, /--bg:#0b0e14/);
  assert.match(css, /--fg:#e9edf7/);
  assert.match(css, /--accent:#6d5cff/);
  ...
  assert.match(css, /--shadow-floating:0 18px 54px rgba\(2,5,12,.6\)/);
...
test("artifact SDK uses design-token aliases for annotation highlight and shadow UI", () => {
  ...
  assert.match(js, /--canvasflow-accent:#6d5cff/);
  assert.match(js, /--canvasflow-annotate-outline:2px solid var\(--canvasflow-accent\)/);
  assert.match(js, /el\.style\.outline\s*=\s*["']var\(--canvasflow-annotate-outline,2px solid #6d5cff\)["']/);
  assert.match(js, /el\.style\.outlineOffset\s*=\s*["']var\(--canvasflow-annotate-offset,2px\)["']/);
  assert.match(js, /--fg-faint:#6d7791/);
...
test("chrome top bar follows the design mock wordmark and overflow menu treatment", async () => {
  ...
  assert.match(html, /class="brand-mark">canvas/);
  assert.match(html, /class="brand-support">review/);
  assert.match(html, /class="brand-flow">flow/);
  assert.match(css, /\.brand-mark\{[^}]*font-family:var\(--font-mono\)/);
...
test("overflow menu offers publishing an ht-ml.app link via a share dialog", async () => {
  ...
  assert.match(html, /third-party hosting service, not part of CanvasFlow/);
...
test("composer send is a split button with a send-and-end option", async () => {
  ...
  assert.match(css, /\.send-caret\{[^}]*border-left:1px solid rgba\(11,14,20,.28\)/);
});
...
test("artifact SDK audits layout after fonts and ResizeObserver settle", () => {
  ...
  assert.match(js, /type:\s*["']canvasflow:layoutWarnings["']/);
...
test("artifact SDK reports its scroll position and restores it on request", () => {
  ...
  assert.match(js, /type:\s*["']canvasflow:scroll["']/);
  assert.match(js, /window\.scrollX/);
  assert.match(js, /window\.scrollY/);
  assert.match(js, /msg\.type === ["']canvasflow:restoreScroll["']/);
...
test("chrome remembers the artifact scroll position across reloads", async () => {
  ...
  assert.match(js, /msg\.type === ["']canvasflow:scroll["']/);
  assert.match(js, /type:\s*["']canvasflow:restoreScroll["']/);
  assert.match(js, /x:\s*lastScroll\.x,\s*y:\s*lastScroll\.y/);
});

test("chrome ignores CanvasFlow postMessages not sent by the artifact iframe", async () => {
  const js = await chromeClientSource();
...
test("chrome restores queued prompts from tab storage after reload", async () => {
  ...
  assert.match(js, /canvas-flow:queued:/);
...
test("/health reports the server version so clients can detect upgrades", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "canvasflow-serve-"));
...
test("session URLs can disable the layout gate for one open", async () => {
  ...
  try {
    ...
    assert.match(chrome, /<body class="canvasflow">/);
...
test("/artifact serves files copied under the artifact directory", async () => {
  const parent = await mkdtemp(path.join(tmpdir(), "canvasflow-serve-"));
  const dir = path.join(parent, ".canvasflow");
...
test("/chrome.css serves the extracted chrome stylesheet", async () => {
  ...
  try {
    ...
    assert.match(normalizeCssForAssertions(body), /--bg:#0b0e14/);
    assert.match(
...
test("GET /api/:key/export reports unresolved local asset warning count", async () => {
  ...
  try {
    ...
    assert.equal(exportRes.status, 200);
    assert.equal(exportRes.headers.get("x-canvasflow-export-warning-count"), "1");
    assert.equal(exportRes.headers.get("x-canvasflow-export-notice-count"), "0");
    assert.equal(exportRes.headers.get("x-canvasflow-export-warnings"), null);
...
test("GET /api/:key/export counts notices separately from unresolved assets", async () => {
  ...
  try {
    ...
    assert.equal(exportRes.status, 200);
    assert.equal(exportRes.headers.get("x-canvasflow-export-warning-count"), "0");
    assert.equal(exportRes.headers.get("x-canvasflow-export-notice-count"), "1");
...
test("POST /api/:key/share publishes the local-inlined artifact to ht-ml.app", async () => {
  ...
  const previousApiUrl = process.env.CANVAS_FLOW_HTML_APP_API_URL;
  process.env.CANVAS_FLOW_HTML_APP_API_URL = `http://127.0.0.1:${htmlApp.port}`;
  ...
  try {
    ...
    await server.close();
    await htmlApp.close();
    restoreEnv("CANVAS_FLOW_HTML_APP_API_URL", previousApiUrl);
    await rm(dir, { recursive: true, force: true });
  }
});
...
test("resolveIdleTimeoutMs defaults, parses, and only explicit opt-outs disable", () => {
  ...
  assert.equal(resolveIdleTimeoutMs({ CANVAS_FLOW_IDLE_TIMEOUT_MS: "" }), 30 * 60_000);
  assert.equal(resolveIdleTimeoutMs({ CANVAS_FLOW_IDLE_TIMEOUT_MS: "5000" }), 5000);
  assert.equal(resolveIdleTimeoutMs({ CANVAS_FLOW_IDLE_TIMEOUT_MS: "0" }), null);
  assert.equal(resolveIdleTimeoutMs({ CANVAS_FLOW_IDLE_TIMEOUT_MS: "off" }), null);
  assert.equal(resolveIdleTimeoutMs({ CANVAS_FLOW_IDLE_TIMEOUT_MS: "-1" }), 30 * 60_000);
  assert.equal(resolveIdleTimeoutMs({ CANVAS_FLOW_IDLE_TIMEOUT_MS: "30000ms" }), 30 * 60_000);
  assert.equal(resolveIdleTimeoutMs({ CANVAS_FLOW_IDLE_TIMEOUT_MS: "later" }), 30 * 60_000);
});
...
test("an agent-initiated end via the file-based route reopens normally without the reopen flag", async () => {
  ...
  try {
    ...
    // `canvas-flow end <file>` uses the file-based route - agent-initiated.
    await fetch(`${base}/api/end`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ file: artifact }),
...
test("hasLiveReloadRootOptIn detects the data attribute and meta opt-in", () => {
  ...
  assert.equal(hasLiveReloadRootOptIn(`<html data-canvasflow-live-reload-root><body></body></html>`), true);
  assert.equal(
    hasLiveReloadRootOptIn(`<html><head><meta name="canvasflow-live-reload" content="root"></head></html>`),
    true,
  );
});

test("hasLiveReloadRootOptIn ignores commented and text data attribute mentions", () => {
  assert.equal(hasLiveReloadRootOptIn(`<!-- <html data-canvasflow-live-reload-root> -->`), false);
  assert.equal(
    hasLiveReloadRootOptIn(`<html><body><code>data-canvasflow-live-reload-root</code></body></html>`),
    false,
  );
});

test("resolveWatchTarget defaults to the artifact file so large sibling trees aren't scanned", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "canvasflow-watch-"));
  const artifact = path.join(dir, "artifact.html");
  await writeFile(artifact, "<!doctype html><html><body></body></html>");
  try {
...
test("resolveWatchTarget upgrades to the artifact directory when data-canvasflow-live-reload-root opts in", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "canvasflow-watch-"));
  const artifact = path.join(dir, "artifact.html");
  await writeFile(artifact, `<!doctype html><html data-canvasflow-live-reload-root><body></body></html>`);
  try {
    const target = await resolveWatchTarget({ file: artifact, key: "abc" });
...
test("resolveWatchTarget falls back to file-only when the artifact can't be read", async () => {
  const target = await resolveWatchTarget({
    file: path.join(tmpdir(), `canvasflow-missing-artifact-${process.hrtime.bigint()}.html`),
    key: "abc",
  });
  assert.equal(target.scope, "file");
});
...
test("concurrent same-session opens create only one file watcher", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "canvasflow-watch-race-"));
  const artifact = path.join(dir, "artifact.html");
...
test("/health and / stay responsive after opening two back-to-back sessions", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "canvasflow-back-to-back-"));
...
test("server debug logger receives session and watcher lifecycle events", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "canvasflow-debug-"));
  const artifact = path.join(dir, "artifact.html");
  await writeFile(artifact, "<!doctype html><html><body></body></html>");
...
test("ended session shows an overlay card over the dimmed chrome", async () => {
  ...
  assert.match(css, /\.ended-overlay\{[^}]*background:rgba\(6,9,15,.88\)/);
...
test("layout gate curtain reuses the ended overlay card styling", async () => {
  ...
  assert.match(html, /<body class="canvasflow layout-gate-active">/);
  assert.match(
    html,
  ...
  assert.match(noGateHtml, /<body class="canvasflow">/);
...
test("annotation card queues and sends immediately on Ctrl+Enter or Cmd+Enter", () => {
  ...
  assert.match(js, /class="canvasflow-hint"/);
  assert.match(js, /\+Enter to send now/);
  assert.match(js, /\.canvasflow-annotation-card \.canvasflow-hint\{/);
});
