...
async function createChromeHarness({
  ...
  element("canvasflow-session").textContent = JSON.stringify(sessionData);
  ...
  return {
    ...
    queued() {
      return JSON.parse(storage.get("canvas-flow:queued:abc") || "[]");
    },
    runTimers,
    srcLoads,
  };
...
test("chrome client replaces queued prompts with the same internal key", async () => {
  ...
  chrome.sendFrameMessage({
    type: "canvasflow:queuePrompt",
    prompt: {
      prompt: "Use plan A",
      selector: "input#plan-a",
      tag: "choice",
      text: "Plan A",
      _canvasflowQueueKey: "plan",
    },
  });
  chrome.sendFrameMessage({
    type: "canvasflow:queuePrompt",
...
test("chrome client posts layout warnings from the artifact iframe", async () => {
  ...
  chrome.sendFrameMessage({
    type: "canvasflow:layoutWarnings",
    layout_warnings: [
      {
        selector: "html",
...
test("chrome client surfaces export warnings from the server response", async () => {
  const chrome = await createChromeHarness({
    fetchImpl: async () => ({
      ...
      headers: {
        get(name) {
          if (name.toLowerCase() === "x-canvasflow-export-warning-count") return "1";
          return null;
        },
      },
      blob: async () => ({}),
...
test("chrome client surfaces export notices from the server response", async () => {
  const chrome = await createChromeHarness({
    fetchImpl: async () => ({
      ...
      headers: {
        get(name) {
          if (name.toLowerCase() === "x-canvasflow-export-warning-count") return "0";
          if (name.toLowerCase() === "x-canvasflow-export-notice-count") return "1";
          return null;
        },
      },
      blob: async () => ({}),
...
test("chrome client includes export notices alongside unresolved assets", async () => {
  const chrome = await createChromeHarness({
    fetchImpl: async () => ({
      ...
      headers: {
        get(name) {
          if (name.toLowerCase() === "x-canvasflow-export-warning-count") return "2";
          if (name.toLowerCase() === "x-canvasflow-export-notice-count") return "1";
          return null;
        },
      },
...
test("layout gate reveals after a clean audit result", async () => {
  ...
  chrome.sendFrameMessage({ type: "canvasflow:layoutWarnings", layout_warnings: [] });
  await flushPromises();
...
test("chrome client strips the internal queue key before posting prompts", async () => {
  ...
  assert.equal(chrome.postedToFrame.at(-1).type, "canvasflow:requestSnapshot");

  chrome.sendFrameMessage({ type: "canvasflow:snapshot", snapshot: "uid=1 body" });
  await flushPromises();
...
test("Cmd/Ctrl+I toggles annotation mode from the chrome document, regardless of focus", async () => {
  ...
  assert.equal(chrome.postedToFrame.at(-1).type, "canvasflow:setAnnotationMode");
...
test("chrome client toggles annotation mode when the artifact SDK requests it via postMessage", async () => {
  ...
  chrome.sendFrameMessage({ type: "canvasflow:toggleAnnotationMode" });
...
test("chrome client ignores annotation mode toggles after the session ends", async () => {
  ...
  chrome.sendFrameMessage({ type: "canvasflow:endSession" });
  await flushPromises();
