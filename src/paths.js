...
// Address the server binds to (CANVAS_FLOW_HOST). Defaults to loopback. A wildcard value
// (0.0.0.0 or ::) binds every interface.
export function bindHost(env = process.env) {
  return env.CANVAS_FLOW_HOST?.trim() || LOOPBACK_HOST;
}
...
// Hostname written into the session URLs the server generates (CANVAS_FLOW_LINK_HOST).
// Defaults to the host the CLI dials.
export function linkHost(env = process.env) {
  return env.CANVAS_FLOW_LINK_HOST?.trim() || clientHost(env);
}
...
export function stateDir() {
  return process.env.CANVAS_FLOW_STATE_DIR || path.join(os.homedir(), ".canvas-flow");
}
...
export function defaultPort() {
  return Number(process.env.CANVAS_FLOW_PORT || 4387);
}
