...
export const PLAYBOOKS = [
  {
    ...
    canvasflow_notes: [
      "A CanvasFlow diagram should invite precise annotation: make modules, edges, and captions easy to click and discuss.",
  ...
  },
  {
    ...
    canvasflow_notes: [
      "A CanvasFlow table should make individual rows easy annotation targets.",
  ...
  },
  {
    ...
    canvasflow_notes: [
      "A CanvasFlow comparison should let the user annotate the exact option or tradeoff they want changed.",
  ...
  },
  {
    ...
    canvasflow_notes: [
      "A CanvasFlow plan should make a plan and its uncertainties easy to annotate before code exists.",
    ],
  },
  {
    id: "code",
    use_when: "Render source code, code files, patches, PR diffs, and before/after code inside CanvasFlow artifacts",
    choose: [
...
\`\`\``,
    ...
    canvasflow_notes: [
      "A CanvasFlow code artifact should make each file, hunk, and relevant line easy to annotate precisely.",
  ...
  },
  {
    ...
    design_rules: [
      "Native controls - radios, checkboxes, text inputs, selects, textareas, buttons, options, labels, disclosure summaries, and contenteditable regions - are interactive automatically: clicks toggle, focus, and type instead of annotating, so they do not need data-canvasflow-action. Build choice and option UIs from these whenever you can.",
      "For reversible choices, do not call window.canvasflow.queuePrompt() from radio change handlers or option click handlers. Those handlers should only update local selected state.",
      "Use a per-question form submit or explicit Queue answer button to read the current values and call window.canvasflow.queuePrompt() exactly once for the final answer.",
      "Put data-canvasflow-action only on custom (non-native) elements that should act like a feedback control - typically a styled div or span you made clickable - so CanvasFlow does not annotate it and shows a pointer cursor instead.",
      "Use data-canvasflow-question on a question wrapper or pass queueKey when multiple pre-send updates should replace the prior unsent answer for the same question.",
      ...
      "Call window.canvasflow.sendQueuedPrompts() only when the control should immediately send committed feedback instead of waiting for the user to press Send to Agent.",
    ...
    canvasflow_notes: [
      "CanvasFlow is strongest when the artifact becomes a focused review surface and not just a static page.",
      'A native single-choice question should submit the final value: `<form data-canvasflow-question="plan" onsubmit="event.preventDefault(); const choice = new FormData(event.currentTarget).get(\'plan\'); if (choice) window.canvasflow.queuePrompt(\'Use the \' + choice + \' plan\', { tag: \'choice\', text: \'Plan: \' + choice, element: event.currentTarget, data: { question: \'plan\', answer: choice } });"><label><input type="radio" name="plan" value="Starter"> Starter</label><label><input type="radio" name="plan" value="Pro"> Pro</label><button type="submit">Queue this answer</button></form>`.',
      "A custom choice UI should make option buttons update local state, then use a separate Queue answer button with data-canvasflow-action to queue the final selected value.",
      "Use window.canvasflow.queuePrompt for user intent, not internal analytics or UI-only state changes.",
  ...
  },
  {
    ...
    canvasflow_notes: [
      "A CanvasFlow slide deck can still collect feedback, but each prompt should refer to a slide or decision.",
