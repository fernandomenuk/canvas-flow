// ponytail: no telemetry endpoint by default. Set CANVAS_FLOW_UMAMI_HOST + CANVAS_FLOW_UMAMI_WEBSITE_ID to enable.
const HARDCODED_FALLBACK_HOST = "";
const UMAMI_PATH = "/api/send";
const DEFAULT_HOSTNAME = "cli";
const DEFAULT_TITLE = "Canvas Flow CLI";
const DEFAULT_REQUEST_TIMEOUT_MS = 1_000;

export function resolveTelemetryConfig(input) {
  const optOut = String(input.env.CANVAS_FLOW_TELEMETRY || "")
    .trim()
    .toLowerCase();
  ...
  const websiteID = String(input.env.CANVAS_FLOW_UMAMI_WEBSITE_ID || "").trim() || input.buildWebsiteID.trim();
  if (!websiteID) {
    return { enabled: false, host: "", websiteID: "" };
  }

  const host =
    String(input.env.CANVAS_FLOW_UMAMI_HOST || "").trim() || input.buildHost.trim() || HARDCODED_FALLBACK_HOST;
  return { enabled: true, host, websiteID };
}

export function getBuildTimeUmamiHost() {
  return process.env.CANVAS_FLOW_BUILD_UMAMI_HOST || "";
}

export function getBuildTimeUmamiWebsiteID() {
  return process.env.CANVAS_FLOW_BUILD_UMAMI_WEBSITE_ID || "";
}
