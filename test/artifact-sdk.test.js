...
function matchesSelector(el, selector) {
  ...
  if (selector === "[data-canvasflow-question]") return el.getAttribute("data-canvasflow-question") !== null;
...
test("deriveCanvasFlowQueueKey uses explicit queueKey first", () => {
  const input = node("input", { type: "radio", name: "plan" });

  assert.equal(deriveCanvasFlowQueueKey(input, { queueKey: "deployment-plan" }), "deployment-plan");
});

test("deriveCanvasFlowQueueKey allows explicit empty queueKey to suppress derivation", () => {
  const button = node("button");
  node("section", { "data-canvasflow-question": "deployment-plan" }, [button]);

  assert.equal(deriveCanvasFlowQueueKey(button, { queueKey: "" }), "");
});

test("deriveCanvasFlowQueueKey groups controls inside data-canvasflow-question", () => {
  const first = node("button");
  const second = node("button");
  node("section", { "data-canvasflow-question": "deployment-plan" }, [first, second]);

  assert.equal(deriveCanvasFlowQueueKey(first), "question:deployment-plan");
  assert.equal(deriveCanvasFlowQueueKey(second), "question:deployment-plan");
});

test("deriveCanvasFlowQueueKey groups radio options by scoped group name", () => {
  ...
  assert.equal(deriveCanvasFlowQueueKey(planA), "radio:form:deploy:plan");
  assert.equal(deriveCanvasFlowQueueKey(planB), "radio:form:deploy:plan");
});

test("deriveCanvasFlowQueueKey keeps same radio names independent across scopes", () => {
  ...
  assert.notEqual(deriveCanvasFlowQueueKey(first), deriveCanvasFlowQueueKey(second));
});

test("deriveCanvasFlowQueueKey does not infer plain button grouping without question metadata", () => {
  const button = node("button");

  assert.equal(deriveCanvasFlowQueueKey(button), "");
});

test("deriveCanvasFlowQueueKey keys checkbox toggles per checkbox, not per group", () => {
...
test("deriveCanvasFlowQueueKey does not collide checkbox default values", () => {
...
test("deriveCanvasFlowQueueKey keys named selects as fields", () => {
  const select = node("select", { name: "region" });
  node("form", { id: "deploy" }, [select]);

  assert.equal(deriveCanvasFlowQueueKey(select), "field:form:deploy:region");
});
