...
// `canvas-flow <file>` invocation re-spawns a fresh server and adopts resumable sessions from
// state.json. Browser-ended sessions still require the explicit --reopen opt-in. Set
// CANVAS_FLOW_IDLE_TIMEOUT_MS to 0/off to disable, or to a custom millisecond budget.
export function resolveIdleTimeoutMs(env = process.env) {
  const raw = env.CANVAS_FLOW_IDLE_TIMEOUT_MS?.trim();
...
export async function serve({
  ...
  const verbose = debug || process.env.CANVAS_FLOW_DEBUG === "1";
  const writeLog = typeof log === "function" ? log : (line) => process.stderr.write(`${line}\n`);
  const logEvent = verbose ? (line) => writeLog(`[canvasflow] ${line}`) : null;
  let publicPort = port;

  app.use(express.json({ limit: "2mb" }));

  app.get("/health", (req, res) => {
    res.json({ ok: true, app: "canvas-flow", version });
  });
  ...
  app.post("/api/sessions", async (req, res, next) => {
    try {
      ...
      // `canvas-flow <file>` is the exact behavior this route exists to prevent - require an
      // explicit `reopen` opt-in instead of reviving it automatically. Agent-initiated ends
      // (`canvas-flow end`) keep reviving on the next open, same as before this change.
  ...
  // hosting service not part of CanvasFlow, returning the share URL. Publishing sends the artifact
  ...
  app.get(/^\/artifact\/([^/]+)\/index\.html$/, async (req, res, next) => {
    try {
      ...
      res.type("html").send(injectCanvasFlowSdk(html, key));
    } catch (error) {
      next(error);
    }
  });
...
// `data-canvasflow-live-reload-root` attribute on its root element or
// `<meta name="canvasflow-live-reload" content="root">`.
  ...
  try {
    ...
    if (hasLiveReloadRootOptIn(html)) {
      return {
        ...
        options: {
          ...baseOptions,
          ignored: /(^|[/\\])(\.git|node_modules|dist|build|\.canvas-flow)([/\\]|$)/,
        },
      };
    }
  } catch {
...
export function hasLiveReloadRootOptIn(html) {
  ...
  if (/<html\b[^>]*\sdata-canvasflow-live-reload-root(?:[\s=>/]|$)[^>]*>/i.test(searchableHtml)) return true;
  return /<meta\b(?=[^>]*name=["']canvasflow-live-reload["'])(?=[^>]*content=["']root["'])[^>]*>/i.test(searchableHtml);
}
...
export function createChromeHtml(session, { layoutGateEnabled = true } = {}) {
  ...
  const bodyClass = layoutGateEnabled ? "canvasflow layout-gate-active" : "canvasflow";
...
void key;
const deriveQueueKey=${deriveCanvasFlowQueueKey.toString()};
