...
export function createDesignOutput() {
  return {
    ...
    design: {
      summary:
        "Use this CanvasFlow CDN fallback only if (1) the user gave no design direction and (2) you already inspected the project the artifact is about - the subject or product whose content or UI it represents, which may differ from your current working directory - and found no design system or style conventions to match. If you have not checked the subject project yet, check first. CanvasFlow does not auto-inject any design system; artifacts stay portable HTML. The strict priority order is: (1) a look or named design system the user asked for; (2) the subject project's design system or style conventions - look for a Tailwind or theme config, shared CSS variables or design tokens, a component library, brand assets, or existing styled pages. If the artifact previews, proposes, or mocks a specific app's UI, render it in that app's own design system so it faithfully shows the product, even when you are running in a different repo; (3) this Tailwind CSS browser runtime v4 + DaisyUI v5 + themes snippet - paste the CDN snippet below into your `<head>` and prefer the CDN snippet over hand-writing styles unless explicitly instructed otherwise by the user.",
      ...
      layout_safety_note:
        "Optional copy-paste CSS for artifacts with dense nested grid/flex layouts, badges, wide monospace or pixel fonts, or local media. Paste it into the artifact yourself when useful. CanvasFlow never auto-injects it, so direct-open portability stays intact.",
      other_design_systems:
        "If the user asks for a different design system (Bootstrap, custom CSS, plain HTML, etc.), use that instead - CanvasFlow does not require DaisyUI.",
    },
    diagram_tooling: {
      use_when:
    ...
    theme_usage: [
      'Default to `<html data-theme="luxury">` - it matches the CanvasFlow look. Pick a different theme from the list below only when the user asked for one or the content clearly calls for it.',
