...
const DESCRIPTION =
  "Canvas Flow helps agents turn rich HTML artifacts into collaborative human review surfaces. Whenever you are about to give user a complex response that will be easier to understand via a rich / interactive page, consider using Canvas Flow. " +
  "First generate an interactive HTML artifact according to user request, then run `canvas-flow <html-file>` so the user can visually review it, annotate elements or selected text, queue prompts, and send feedback back through `canvas-flow poll`.";
// Inlined at build time from package.json; falls back to reading package.json so source-run tests work.
export const VERSION =
  process.env.CANVAS_FLOW_BUILD_VERSION ||
...
export async function run(argv) {
  ...
  try {
    await runAxiCli({
      ...
      home: async () =>
        createHomeOutput({
          bin: process.argv[1] || "canvas-flow",
...
export function createHomeOutput({ bin, sessions, includeSessions = true }) {
  return {
    ...
    help: [
      "Run `canvas-flow <html-file>` to open or resume a Canvas Flow session. If the user explicitly ended the session from the browser, this refuses to reopen it and explains why instead of reopening uninvited - pass `--reopen` only when the user asks for further review or something important needs their visual attention",
      "Unless the user specifies another location, create HTML artifacts in the current working directory under `.canvasflow/`",
      "CanvasFlow serves the html file through a local express.js server. If your html needs to reference other filesystem assets such as images, CSS, fonts, and local scripts, copy them into the same directory as the HTML file, then reference them with relative paths from that directory. Never prepend `/` to those asset paths - root paths won't work",
      "Run `canvas-flow poll <html-file>` to wait for user feedback or browser-reported layout_warnings. It long-polls and stays silent until the user sends feedback, ends the session, or the real browser reports fresh layout_warnings, so leave it running - never kill it. Fix and re-check fresh error-severity layout_warnings before involving the human; if the poll says every current warning is persistent or low-severity, proceed with a note instead of looping. If your harness limits how long a foreground command may run, run the poll as a background task; if it gets killed or times out anyway, just re-run it - queued feedback is never lost. When it reports the session ended, stop polling and do not reopen it uninvited - deliver remaining updates in this conversation instead",
      "Run `canvas-flow end <html-file>` to end a session as the agent - ending it this way still allows a plain reopen later. When the user ends it from the browser instead, a later `canvas-flow <html-file>` refuses to reopen it without `--reopen`",
      "Run `canvas-flow export <html-file> [--out <path>]` to write a portable copy of the artifact - one HTML file with its LOCAL assets inlined - so it opens with no CanvasFlow server and no sibling files. Remote CDN/font references are left as links, so it needs network to render those. Users can also export from the browser chrome's overflow menu",
      "Run `canvas-flow share <html-file> [--password <pw>] [--token <t>]` to publish the artifact on ht-ml.app (https://ht-ml.app), a third-party hosting service not part of CanvasFlow, and get back a visitable URL. Shares are PUBLIC by default, so anyone with the link can open them. Pass --password to publish a PRIVATE password-protected page; viewers must supply the password to view. Local assets are inlined; remote refs load over the network. It returns the url plus a secret update_key for managing the page later. Use --token or CANVAS_FLOW_HTML_APP_TOKEN only when you have an optional bearer token; it is never required. Users can also publish from the browser chrome's overflow menu",
      "Run `canvas-flow stop` to shut down the background server (it also self-stops when idle or after the last session ends with nothing connected)",
      `Run \`canvas-flow playbook <playbook_id>\` for focused artifact guidance. ${PLAYBOOK_ROUTER_HELP}`,
      DESIGN_SYSTEM_HINT,
      "Use canvas-flow when the user asks for a visual artifact, HTML explainer, interactive prototype, review surface, product or technical plan, comparison, report, or browser-based feedback loop",
    ],
  };
}
...
export function createPlaybookOutput(args) {
  ...
  if (!id) {
    return {
      playbooks: listPlaybooks(),
      help: ["Run `canvas-flow playbook <playbook_id>` for focused artifact guidance", PLAYBOOK_ROUTER_HELP],
    };
  }
  ...
  if (!playbook) {
    throw new AxiError(`Unknown playbook: ${id}`, "VALIDATION_ERROR", [
      `Run \`canvas-flow playbook\` to list known IDs: ${playbookIds().join(", ")}`,
    ]);
  }
...
export function createUserEndedOpenOutput({ file, url }) {
  return {
    ...
    next_step: `The user explicitly ended this Canvas Flow session from the browser, so \`canvas-flow ${file}\` did not reopen it. Do not reopen unless the user asks for further review or something genuinely important needs their visual attention - deliver routine updates directly in this conversation instead. When reopening is warranted, run \`canvas-flow ${file} --reopen\`.`,
  };
}
...
async function openCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `canvas-flow <html-file>`"]);
  }
...
export function shouldOpenBrowser(args, env) {
  return !args.includes("--no-open") && env.CANVAS_FLOW_NO_OPEN !== "1";
}

async function pollCommand(args) {
  const file = firstPositionalArg(args, ["--agent-reply", "--timeout-ms"]);
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `canvas-flow poll <html-file>`"]);
  }
  const absolute = await canonicalFile(file);
...
export function pollWaitBannerText(file) {
  return (
    `[canvas-flow] Long-polling for user feedback or layout_warnings on ${file}. This stays silent until the user sends feedback, ends the session, or the browser reports fresh layout_warnings - leave it running. ` +
    `If it gets killed or times out, re-run \`canvas-flow poll ${file}\` - queued feedback is never lost.`
  );
}

export function pollWaitTickText(elapsedMs) {
  const minutes = Math.round(elapsedMs / 60_000);
  return `[canvas-flow] Still waiting for user feedback (${minutes}m). Also waiting for fresh layout_warnings. Leave this running until the user acts or the browser reports fresh layout_warnings.`;
}

export function pollInterruptedText(file) {
  return (
    `[canvas-flow] Poll interrupted before user feedback arrived. The user may still be reviewing - ` +
    `re-run \`canvas-flow poll ${file}\` to keep waiting; queued feedback is never lost.`
  );
}
...
export function createPollOutput({ file, response }) {
  if (response.status === "missing") {
    throw new AxiError("No active Canvas Flow session for this file", "NOT_FOUND", [
      `Run \`canvas-flow ${file}\` first`,
    ]);
  }
  ...
  return {
    ...
    next_step: `No user feedback arrived before the optional timeout. Run \`canvas-flow poll ${file}\` without --timeout-ms to wait indefinitely - queued feedback is never lost, so re-running the poll is always safe.`,
  };
}
...
function createFeedbackNextStep(file, layoutWarnings, sessionEnded, endedBy) {
  ...
  if (sessionEnded) {
    ...
    if (endedBy === "user") {
      return `${layoutNote}This was the last feedback before the user ended the session. Stop polling ${file} and do not reopen it - deliver any remaining updates directly in this conversation instead. Only run \`canvas-flow ${file} --reopen\` if the user explicitly asks for further review or something genuinely important needs their visual attention.`;
    }
    return `${layoutNote}This was the last feedback before the Canvas Flow session ended. Stop polling ${file}. Deliver any remaining updates directly in this conversation, or run \`canvas-flow ${file}\` to open a fresh session if the user needs further visual review.`;
  }
  const layoutPrefix =
    count > 0 ? layoutWarningsPrefix(file, layoutWarnings) : `Apply the requested changes to ${file}. `;
  return `${layoutPrefix}Do not respond to the user just yet. Now you must run \`canvas-flow poll ${file} --agent-reply "<message for the user>"\` without --timeout-ms unless the user ended the session. The poll waits silently until the user sends more feedback, ends the session, or reports fresh layout_warnings - never kill it. If your harness limits how long a foreground command may run, run the poll as a background task; if it still gets killed or times out, just re-run it - queued feedback is never lost.`;
}
...
function layoutWarningsPrefix(file, layoutWarnings) {
  ...
  return `${count} layout warning${plural} detected - fix horizontal overflow or clipped text in ${file}, then re-check in the browser before involving the human. CanvasFlow live-reloads the artifact automatically after you save, so you do not need to re-run \`canvas-flow ${file}\` for this. `;
}

function createEndedNextStep(file, endedBy) {
  if (endedBy === "user") {
    return `The user ended this Canvas Flow session. Stop polling ${file} - do not run \`canvas-flow ${file}\` to reopen it. Deliver any remaining updates directly in this conversation instead. Only reopen with \`canvas-flow ${file} --reopen\` if the user explicitly asks for further review or something genuinely important needs their visual attention.`;
  }
  return `This Canvas Flow session for ${file} has ended. Stop polling. Deliver any remaining updates directly in this conversation, or run \`canvas-flow ${file}\` to open a fresh session if the user needs further visual review.`;
}

async function endCommand(args) {
  const file = firstPositionalArg(args);
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `canvas-flow end <html-file>`"]);
  }
  const absolute = await canonicalFile(file);
  const baseUrl = await ensureServer();
...
// as-is for the browser to load, so the export needs network to render those. CanvasFlow makes no
...
async function exportCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `canvas-flow export <html-file>`"]);
  }
  await assertHtmlFile(file);
  const absolute = await canonicalFile(file);
...
export function createExportOutput({ source, output, html, warnings }) {
  ...
  if (unresolved.length) {
    ...
    result.next_step = `Wrote ${output} with export notices (see notices). Open it directly or host it anywhere - it needs no CanvasFlow server. Local assets are inlined; remote CDN/font references are left as links, so it needs network to render those.`;
  } else {
    result.next_step = `Wrote ${output}. Open it directly or host it anywhere - it needs no CanvasFlow server. Local assets are inlined; remote CDN/font references are left as links, so it needs network to render those.`;
  }
  return result;
}
...
// sending the artifact to ht-ml.app's servers. The service is not part of CanvasFlow, needs no
...
async function shareCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `canvas-flow share <html-file>`"]);
  }
  await assertHtmlFile(file);
  const absolute = await canonicalFile(file);
...
export function createShareOutput({ source, site, warnings, passwordProtected = false }) {
  ...
  const hostNote =
    "ht-ml.app (https://ht-ml.app), a third-party host not part of CanvasFlow, hosts the page, so it needs no CanvasFlow server.";
  if (unresolved.length) {
    result.next_step =
...
// Explicitly shut down the running Canvas Flow server. Unlike `end` (which closes a single
...
export async function shutdownServerOnPort(
  port,
  {
    ...
    processMatchesCanvasFlow = processOnPortMatchesCanvasFlow,
  } = {},
) {
  ...
  if (!(await canControlServerOnPort(port, health, processMatchesCanvasFlow))) {
    return { server: { status: "not-canvasflow", port } };
  }
...
async function setupCommand(args) {
  if (args.length !== 1 || args[0] !== "hooks") {
    throw new AxiError("Unknown setup action", "VALIDATION_ERROR", ["Run `canvas-flow setup hooks`"]);
  }

  const errors = [];
  installSessionStartHooks({
    marker: "canvas-flow",
    binaryNames: ["canvas-flow"],
    distEntrypoints: ["dist/cli.mjs", "bin/canvas-flow.js"],
  ...
  if (errors.length > 0) {
    throw new AxiError("Failed to install canvas-flow agent hooks", "SERVER_ERROR", errors);
  }

  return {
    hooks: { status: "installed", integrations: "Claude Code, Codex, OpenCode, GitHub Copilot CLI" },
    help: ["Restart your agent session to receive canvas-flow ambient context"],
  };
}
...
export function createCopilotCliAmbientContextScript(command = "canvas-flow") {
  return [
    ...
    'const prefix = result.status === 0 ? "## AXI ambient context: canvas-flow\\n" : "## AXI ambient context: canvas-flow\\nerror: canvas-flow ambient context failed: ";',
    "console.log(JSON.stringify({ additionalContext: prefix + text }));",
  ].join(" ");
}

export function createCopilotCliSessionStartHook(command = "canvas-flow", timeoutSec = 10) {
...
export function installCopilotCliSessionStartHook({
  ...
  const target = path.join(hookDir, "canvas-flow.json");
  try {
...
function isManagedCopilotCliHook(entry) {
  return (
    ...
    [entry.bash, entry.powershell, entry.command].some(
      (value) => typeof value === "string" && value.includes("canvas-flow"),
    )
  );
}
...
async function serverCommand(args) {
  const port = Number(flagValue(args, "--port") || defaultPort());
  const debug = args.includes("--verbose") || process.env.CANVAS_FLOW_DEBUG === "1";
...
async function assertHtmlFile(file) {
  if (!isHtmlPath(file)) {
    throw new AxiError("Canvas Flow expects an HTML file", "VALIDATION_ERROR", ["Run `canvas-flow <html-file>`"]);
  }
  try {
    await access(file);
  } catch {
    throw new AxiError(`File not found: ${file}`, "NOT_FOUND", [
      "Create the HTML artifact first, then run `canvas-flow <html-file>`",
    ]);
  }
}
...
async function ensureServer({ forceRestart = false } = {}) {
  ...
  if (existing) {
    if (!(await canControlServerOnPort(port, existing, processOnPortMatchesCanvasFlow))) {
      throw new AxiError(`Port ${port} is occupied by a non-CanvasFlow server`, "SERVER_ERROR", [
        `Stop the process using port ${port}, or set CANVAS_FLOW_PORT to another port`,
      ]);
    }
  ...
  throw new AxiError("Canvas Flow server did not start", "SERVER_ERROR", [
    `Run \`canvas-flow server --port ${port}\` to inspect server startup`,
  ]);
}
...
export function shouldRestartServer(currentVersion, healthBody, forceRestart = false) {
  ...
  if (forceRestart && healthBody.app === "canvas-flow") return true;
...
export function shouldKillProcessOnPort(currentVersion, healthBody) {
  if (!healthBody || typeof healthBody !== "object") return false;
  if (typeof healthBody.version !== "string" || healthBody.version === "") return true;
  if (healthBody.app !== "canvas-flow") return false;
  return healthBody.version !== currentVersion;
}

async function canControlServerOnPort(port, healthBody, processMatchesCanvasFlow) {
  if (!healthBody || typeof healthBody !== "object") return false;
  if (healthBody.app === "canvas-flow") return true;
  if (typeof healthBody.version === "string" && healthBody.version !== "") return false;
  return processMatchesCanvasFlow(port);
}
...
// macOS/Linux only - Windows users would need to kill manually, but canvas-flow isn't
...
function processOnPortMatchesCanvasFlow(port) {
  try {
    ...
    for (const line of pids.stdout.split("\n")) {
      ...
      if (command.status === 0 && /canvas-flow/.test(command.stdout)) {
        return true;
      }
    }
  } catch {
...
// run(). In source layout that's `../bin/canvas-flow.js` (which calls run on import). In the
...
export function resolveServerEntry() {
  const binEntry = fileURLToPath(new URL("../bin/canvas-flow.js", import.meta.url));
...
export function createServerSpawnOptions(logFd = null) {
  ...
  return {
    detached: true,
    stdio,
    env: { ...process.env, CANVAS_FLOW_NO_OPEN: "1" },
  };
}
...
export async function fetchJson(url, { retries = 0, retryDelayMs = 250 } = {}) {
  ...
  if (!response.ok) {
    throw new AxiError(`Canvas Flow request failed: ${response.status}`, "SERVER_ERROR");
  }
  try {
    return await response.json();
  } catch {
...
function serverConnectionError() {
  return new AxiError("Canvas Flow server connection failed", "SERVER_ERROR", [
    "Run `canvas-flow server --verbose` or inspect `~/.canvas-flow/server.log` (`CANVAS_FLOW_STATE_DIR/server.log` when set) for server startup or crash diagnostics",
    "Re-run the last `canvas-flow poll <html-file>` command after the server is healthy",
  ]);
}

function pollResponseInterruptedError() {
  return new AxiError("Canvas Flow poll response was interrupted", "SERVER_ERROR", [
    "Run `canvas-flow server --verbose` or inspect `~/.canvas-flow/server.log` (`CANVAS_FLOW_STATE_DIR/server.log` when set) for server startup or crash diagnostics",
    "Re-run the last `canvas-flow poll <html-file>` command after the server is healthy",
  ]);
}
...
const TOP_LEVEL_HELP = `canvas-flow - Canvas Flow AXI\n\nUsage:\n  canvas-flow\n  canvas-flow <html-file> [--no-open] [--no-gate] [--reopen]\n  canvas-flow poll <html-file> [--agent-reply "..."]\n  canvas-flow end <html-file>\n  canvas-flow export <html-file> [--out <path>]\n  canvas-flow share <html-file> [--password <pw>] [--token <t>]\n  canvas-flow stop\n  canvas-flow playbook [playbook_id]\n  canvas-flow design\n  canvas-flow setup hooks\n\n${DESIGN_SYSTEM_HINT}\n\nNote: poll long-polls indefinitely by default until the user sends feedback, ends the session, or the browser reports fresh layout_warnings, staying silent while it waits - never kill it. Fix and re-check fresh error-severity layout_warnings before involving the human; persistent or low-severity findings may be surfaced with a note when the cause is not obvious. Do not pass --timeout-ms during normal agent use; it is for tests and debugging only. If your harness limits how long a foreground command may run, run the poll as a background task; if it gets killed or times out anyway, just re-run it - queued feedback is never lost. When the user ends a session from the browser, stop polling and do not reopen it uninvited - pass --reopen to <html-file> only when the user asks for further review or something important needs their visual attention.\n\n`;

const COMMAND_HELP = {
  open: `Usage: canvas-flow <html-file> [--no-open] [--no-gate] [--reopen]\n\nOpen or resume a Canvas Flow review session for an HTML artifact. Use --no-open when you need to ensure the server/session exists without opening another browser window. Use --no-gate to skip the open-time layout curtain for this browser open. If the user explicitly ended the session from the browser, this refuses to reopen it and returns guidance instead - pass --reopen to force it open when the user asks for further review or something important needs their visual attention. Sessions ended by the agent (\`canvas-flow end\`) reopen normally without the flag.\n`,
  poll: `Usage: canvas-flow poll <html-file> [--agent-reply "..."]\n\nThis command long-polls indefinitely for queued user prompts and browser-reported layout_warnings, then returns them to the agent. It stays silent while it waits - that is normal, never kill it. Fix and re-check fresh error-severity layout_warnings before involving the human; persistent or low-severity findings may be surfaced with a note when the cause is not obvious. Do not pass --timeout-ms during normal agent use; it is for tests and debugging only. If your harness limits how long a foreground command may run, run the poll as a background task and wait for it to finish; if it still gets killed or times out, just re-run it - queued feedback is never lost. Use --agent-reply after applying prior feedback to display your response in Canvas Flow before waiting again. When status is ended, stop polling and do not reopen the session uninvited - deliver remaining updates directly in this conversation instead.\n`,
  end: `Usage: canvas-flow end <html-file>\n\nEnd a Canvas Flow session as the agent. A session ended this way still reopens normally on the next \`canvas-flow <html-file>\`, unlike a user ending it from the browser, which requires --reopen.\n`,
  export: `Usage: canvas-flow export <html-file> [--out <path>]\n\nWrite a portable copy of an artifact: one HTML file with its LOCAL assets inlined (relative-path stylesheets, scripts, images, and fonts become inline <style>/<script> blocks and data URIs). Remote CDN/font references (https URLs) are left as links for the browser to load, so the file needs network to render those. CanvasFlow makes no outbound requests - it only reads local files, confined to the artifact's directory. Defaults to writing <name>.export.html next to the source; pass --out to choose a path. The CanvasFlow annotation SDK is never included in an export.\n`,
  share: `Usage: canvas-flow share <html-file> [--password <pw>] [--token <t>]\n\nPublish the artifact on ht-ml.app (https://ht-ml.app), a third-party hosting service not part of CanvasFlow, and print a visitable URL. Shares are PUBLIC by default: anyone with the link can open the page, and it may be indexed or scraped. Pass --password to publish a PRIVATE password-protected page; viewers must supply the password to view. Builds the same local-inlined HTML as 'export' (local assets inlined; remote CDN/font URLs left as links and are not blocked by CSP on ht-ml.app, but still load over the viewer's network), then POSTs it to ht-ml.app's /v1 API. Creating a site needs no account or API key. The response includes the url plus a secret update_key (shown once) for updating or deleting the page later. Set CANVAS_FLOW_HTML_APP_TOKEN (or pass --token) to attach an optional bearer token; it is never required. The annotation SDK is never included.\n`,
  stop: `Usage: canvas-flow stop [--port <port>]\n\nShut down the background Canvas Flow server. The server also stops itself when no browser or poll has been connected for a while (CANVAS_FLOW_IDLE_TIMEOUT_MS, default 30m) and immediately when the last session ends with nothing connected.\n`,
  playbook: `Usage: canvas-flow playbook [playbook_id]\n\nList focused artifact guidance playbooks, or show one playbook by ID. Known IDs: diagram, table, comparison, plan, code, input, slides.\n\n${PLAYBOOK_ROUTER_HELP}\n\nExamples:\n  canvas-flow playbook\n  canvas-flow playbook diagram\n  canvas-flow playbook input\n`,
  design: `Usage: canvas-flow design\n\nShow a copy-pasteable CDN snippet for Tailwind CSS browser runtime v4 + DaisyUI v5 + themes, Mermaid diagram tooling, a content-to-playbook router, an optional layout safety CSS snippet, plus technical reference for DaisyUI components. ${PLAYBOOK_ROUTER_HELP} CanvasFlow artifacts stay portable HTML. This CDN snippet is the design fallback, not the default: inspect the subject project before falling back, and paste the layout safety CSS only when useful for dense nested grid/flex layouts, badges, wide fonts, or local media. The strict priority order is: (1) if the user asked for a specific look or named design system, follow that; (2) otherwise, match the design system of the project the artifact is about, not necessarily your current working directory. If the artifact previews, proposes, or mocks a specific app's UI, use that app's own design system; (3) only when both come up empty, prefer the CanvasFlow-recommended Tailwind + DaisyUI CDN snippet over hand-writing styles unless explicitly instructed otherwise by the user.\n`,
  setup: `Usage: canvas-flow setup hooks\n\nInstall or repair agent SessionStart hooks for canvas-flow ambient context in Claude Code, Codex, OpenCode, and GitHub Copilot CLI. Restart your agent session afterward to receive the context.\n`,
  server: `Usage: canvas-flow server [--port 4387] [--verbose]\n\nRun the local Canvas Flow server. Pass --verbose (or set CANVAS_FLOW_DEBUG=1) to log session and watcher events to stderr. Detached server output is appended to ~/.canvas-flow/server.log, or CANVAS_FLOW_STATE_DIR/server.log when set, for startup and crash diagnostics.\n\nCANVAS_FLOW_HOST sets the bind address (default 127.0.0.1; a wildcard 0.0.0.0 or :: binds every interface). Binding beyond loopback exposes an unauthenticated server that can read and serve arbitrary local files to anything that can reach it, so only do so on a trusted network. CANVAS_FLOW_LINK_HOST sets the hostname written into generated session links (default: the bind address, or loopback when bound to a wildcard). CANVAS_FLOW_NO_OPEN=1 (or --no-open) suppresses the local browser launch.\n`,
};
